    ...
    <div class="field-grid">
      <div class="field">
        <div class="label-wrap">
          <div class="label-row">
            <label class="field-label">
              ...
              <span class="help-icon" @mouseenter="showTip('width', 'lazer必须设置为skin.ini里ColumnWidth值的1.6倍才不会变形。')"
                @mouseleave="hideTip">
        ...
        <div class="input-wrap">
          <input :value="widthLocal" @input="widthLocal = +($event.target as HTMLInputElement).value" @blur="commitWidth" @keydown.enter="commitWidth" type="number" min="1" max="800" class="num-input" />
        </div>
      ...
      <div class="field">
        <div class="label-wrap">
          <div class="label-row">
            <label class="field-label">
              ...
              <span class="help-icon" @mouseenter="showTip('height', 'lazer推荐32800，否则可能导致尾部变形。stable推荐32767。')"
                @mouseleave="hideTip">
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                  <circle cx="6" cy="6" r="5" stroke="currentColor" stroke-width="1" />
                  <path
        ...
        <div class="input-wrap">
          <input :value="heightLocal" @input="heightLocal = +($event.target as HTMLInputElement).value" @blur="commitHeight" @keydown.enter="commitHeight" type="number" min="1" max="65535" class="num-input" />
        </div>
      </div>
    </div>
    <div class="field">
      <div class="label-wrap">
        <div class="label-row">
          <label class="field-label">
            图片名称
            <span class="help-icon" @mouseenter="showTip('filename', '因lazer加载问题对于此图片来说2x反而不如1x清晰。')"
              @mouseleave="hideTip">
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                <circle cx="6" cy="6" r="5" stroke="currentColor" stroke-width="1" />
                <path
      ...
      <div class="input-wrap input-with-suffix">
        <input v-model="filenameModel" type="text" class="text-input" placeholder="mania-note1H" spellcheck="false" />
...
// 本地缓存值，失焦或回车时才提交
const widthLocal = ref(config.image.width)
const heightLocal = ref(config.image.height)

// 外部 config 变化时同步本地值
watch(() => config.image.width, (v) => { widthLocal.value = v })
watch(() => config.image.height, (v) => { heightLocal.value = v })

function commitWidth() {
  const v = widthLocal.value
  if (v > 800) {
...
function commitHeight() {
  const v = heightLocal.value
  setImageProp('height', Math.max(1, Math.min(65535, v)))
}
